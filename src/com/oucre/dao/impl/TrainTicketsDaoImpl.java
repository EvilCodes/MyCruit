...
@Repository
public class TrainTicketsDaoImpl extends BaseDao<Ticket> implements TrainTicketsDao {
	private Logger logger = Logger.getLogger(this.getClass());
	
	@Override
	public Integer add(Ticket entity) {
		try {
...
/*
	...
	@Override
	public Map<String, Object> findTrainTicketsSearch(Map<String, Object> map, EasyUiPager easyUiPager) {
		try {
			String Hql = "from Ticket where 1=1 ";
			/*if (map.get("uid") != null) {
				Hql += "and uid = " + map.get("uid") + " ";
			}*/
			if (map.get("tfrom") != null) {
				Hql += "and tfrom like '" + map.get("tfrom") + "%' ";
			}
			if (map.get("tto") != null) {
				Hql += "and tto like '" + map.get("tto") + "%' ";
			}
			String orderby = "";
			if (easyUiPager.getOrderby() != null) {
				orderby = " " + easyUiPager.getOrderby() + " ";
			}
			List<Ticket> list = super.findHqlByPager(Hql + orderby,
...
*/
	...
	@Override
	public boolean upd(Ticket entity) {
	...
	public boolean del(Integer id) {
		try {
			super.deleteEntity(new Ticket(id));
			return true;
		} catch (Exception e) {
			logger.error("dao´íÎó", e);
			return false;
