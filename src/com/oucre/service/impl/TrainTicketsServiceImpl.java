...
import com.oucre.dao.TrainTicketsDao;
...
@Service
public class TrainTicketsServiceImpl implements TrainTicketsManagerService {

	@Autowired
	private TrainTicketsDao trainTicketsDao;

	@Override
	public Map<String, Object> findTrainTicketsSearch(Map<String, Object> map, EasyUiPager easyUiPager, User user) {
		if (user != null) {
			return trainTicketsDao.findTrainTicketsSearch(map, easyUiPager);
		}
		return null;
	}

	@Override
	public AjaxJson addTrainTickets(Ticket ticket, User user) {
		AjaxJson a = new AjaxJson();
		if (user != null) {
			if (trainTicketsDao.add(ticket) > 0) {
				a.setSuccess(true);
				a.setMsg("操作成功");
			} else {
				a.setSuccess(false);
	...
	@Override
	public AjaxJson delTrainTickets(Integer id, User attribute) {
		AjaxJson a = new AjaxJson();
		if (ValidateUtil.isInt(id + "")) {
			if (trainTicketsDao.del(id)) {
				a.setSuccess(true);
				a.setMsg("操作成功");
			} else {
				a.setSuccess(false);
